...
export default function Bookings() {
  ...
  const userBookings = reservations.filter((r) => r.userId === user?.id);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {userBookings.map((r) => {
            ...
            return (
              <div
                key={r.id}
                className="bg-gray-800 rounded-xl shadow-md p-4 flex flex-col"
              >
                <img
                ...
                <p>
                  <span className="font-semibold">Time Slot:</span> {r.timeSlot}
                </p>
                <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-gray-800 p-6 rounded-xl w-80 text-center">
            ...
            <div className="flex justify-between gap-4">
              <button
                className="bg-gray-600 hover:bg-gray-700 text-white py-2 px-4 rounded w-1/2"
                onClick={() => setModalOpen(false)}
              >
                No
              </button>
              <button
                className="bg-red-600 hover:bg-red-700 text-white py-2 px-4 rounded w-1/2"
